---
title: DDD — US-PONB-08 — See myself as seekers see me
updated: 2026-09-04
---

# US-PONB-08 — See myself as seekers see me
...
No dedicated acceptance-criteria list in `user-stories.md` §10 for this story — behavior is fully specified by the story statement above and the traces below.
...
- `07-test-artifacts/04-traceability-matrix.md` row for US-PONB-08 cross-references this DDD (applied in the stage-9 traceability pass).
...
**Approach:** Added `getProfilePreviewForOwner` in `provider-profile/infra/preview-profile.ts` which loads the owner's profile view and serializes via `toPublicProfile` with a **forced** viewer role (`anonymous` or `seeker`), skipping phone fetch when the forced anonymous view would omit it. Exposed as `GET /api/provider/me/profile/preview?as=anonymous|seeker` (provider RBAC, read-only, no events). Built `/provider/profile/preview` SSR page showing side-by-side audience panels with clear labels per TC-PONB-08a. Extracted `PublicProfileView.svelte` from the public profile route for reuse; preview mode renders contact/Call affordances visually without live navigation. Entry points: dashboard and edit profile ("Preview as seeker").
