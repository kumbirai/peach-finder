---
title: DDD — US-ANLY-04 — Cause and effect on the chart
updated: 2026-09-04
---

# US-ANLY-04 — Cause and effect on the chart
...
**Supporting modules:** `provider-availability` (`../../05-low-level-design/03-provider-availability/provider-availability-lld.md`); `listing-billing` (`../../05-low-level-design/09-listing-billing/listing-billing-lld.md`)
...
- `07-test-artifacts/04-traceability-matrix.md` row for US-ANLY-04 cross-references this DDD (applied in the stage-9 traceability pass).
- No application code exists yet for this story; this document is the blueprint an implementer builds from, not the implementation.

## 7. Implementation Notes

**Approach:** FR-ANLY-05 chart annotations on the existing `GET /api/analytics/dashboard` path: `provider-availability.listAvailabilityAnnotationEvents` and `listing-billing.listFeaturingActivationsInRange` / `getActiveFeaturingActivatedAt` supply supporting-module data; `domain/chart-annotations.ts` builds summaries ("Went available N× …", "Featured since …") and per-day markers; `infra/chart-annotations-read.ts` loads annotations on every dashboard read (not cached with metric rollups). `ProviderAnalyticsSection.svelte` renders summary list plus symbol+label markers on each trend sparkline (● terracotta = availability, ◆ pine = featured — Never-Color-Alone). Dev seed `/api/dev/analytics-seed?scenario=chart-annotations` for Playwright. Tests: unit (`chart-annotations.test.ts`, `provider-analytics-display.test.ts`), integration (`TC-ANLY-04a`), Playwright (`provider-analytics.e2e.ts` US-ANLY-04 describe).

**Deviation:** None.

**Follow-ups:** US-PRIV-03d raw-event purge integration test at 90-day boundary.
