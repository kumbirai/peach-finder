---
title: DDD — US-AVAIL-05 — No black boxes about my own signals
updated: 2026-09-04
---

# US-AVAIL-05 — No black boxes about my own signals
...
No dedicated acceptance-criteria list in `user-stories.md` §11 for this story — behavior is fully specified by the story statement above and the traces below.
...
- `07-test-artifacts/04-traceability-matrix.md` row for US-AVAIL-05 cross-references this DDD (applied in the stage-9 traceability pass).
...
**Approach:** Extended `GET /api/availability/status/me` via `getAvailabilityTransparencyForOwner` to compose `availability` (including `expiresInSeconds`) with `activeThisWeek` — live evaluation of the four FR-AVAIL-06 signals through `trust-and-safety.evaluateActiveThisWeekSignals`, plus `availabilitySetCount` from this module's history. Provider dashboard `AvailabilityToggle` now shows a live expiry countdown and embeds `ActiveThisWeekTransparency` (signal checklist + earned/neutral headline matching the prototype's `provider-trust-note`).

**Tests:** Unit (`active-this-week-transparency.test.ts`, `availability-expiry-countdown.test.ts`), integration (`availability-transparency.integration.test.ts`), Playwright (`testing/playwright/availability-transparency.e2e.ts` for TC-AVAIL-05a) — seeded dual-role provider sign-in, live-stack-seeded, zero critical/serious axe on the availability hero.
