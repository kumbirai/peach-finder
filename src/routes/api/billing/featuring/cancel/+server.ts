...
import { cancelFeaturingRenewalForOwner } from '$lib/server/modules/listing-billing';
...
export const POST: RequestHandler = async ({ locals }) => {
	const db = getDb();
	const result = await cancelFeaturingRenewalForOwner(db, locals.auth.userId!, new Date());
	if (!result.ok) {
