...
describe('playwright-env', () => {
	...
	it('keeps webServer port aligned with resolved E2E port', () => {
		...
		expect(buildWebServerCommand(port)).toBe(
			`npm run db:migrate && SEED_PACK=seed-core npm run db:seed && node --env-file=.env --import tsx scripts/seed-blocking.ts && node --env-file=.env --import tsx scripts/seed-verification.ts && node --env-file=.env --import tsx scripts/seed-reports.ts && ALLOW_DEV_HELPERS=1 npm run dev -- --host 127.0.0.1 --port ${port}`
		);
